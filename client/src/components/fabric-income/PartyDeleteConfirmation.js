  ...
  const handleDelete = async () => {
    try {
      ...
      if (err.response?.data?.hasAssociatedStock) {
        ...
        if (onError) {
          onError(err.response?.data?.message || 'Failed to delete party');
        }
      }
    }
  };
  ...
  return (
    <Modal show={show} onHide={onHide} centered>
      ...
      <Modal.Body>
        {!hasAssociatedStock ? (
          <>
            <p>Are you sure you want to delete party <strong>{party?.shop_name}</strong>?</p>
          ...
          <>
            <Alert variant="warning">
              ...
              <p>{error}</p>
              <p>This party has {stockCount} associated fabric stock items.</p>
              <hr />
              <p className="mb-0">
                You can either reassign the stock to another party or force delete the party and remove the party reference from stock items.
              </p>
            </Alert>
            ...
            <Form.Check 
              ...
              label="Force delete and remove party reference from stock items"
              checked={forceDelete}
            ...
            <p className="text-danger">
              <strong>Warning:</strong> Force deleting will remove the party reference from all stock items, which may make it difficult to track the origin of the fabric.
            </p>
          </>
        )}
      </Modal.Body>
...
export default PartyDeleteConfirmation;
