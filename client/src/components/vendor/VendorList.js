...
const PartyList = () => {
  ...
  // Fetch partys on component mount and when search/sort/page changes
  useEffect(() => {
    fetchPartys();
  }, [currentPage, searchTerm, sortField, sortDirection]);

  // Fetch partys from the API
  const fetchPartys = async () => {
    try {
      ...
      if (response.data.success) {
        ...
        setTotalPartys(response.data.total || 0);
      } else {
        setError('Failed to fetch partys');
      }
    } catch (err) {
      console.error('Error fetching partys:', err);
      setError('Failed to fetch partys. Please try again.');
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleDelete = async () => {
    ...
    try {
      ...
      if (response.data.success) {
        ...
        setPartys(partys.filter(v => v._id !== partyToDelete._id));
        ...
        <Card.Body>
          <Row>
            <Col md={6}>
              <InputGroup>
                <Form.Control
                  placeholder="Search partys by name, shop or bill number"
                  value={searchTerm}
            ...
            <Col md={6} className="d-flex justify-content-end align-items-center">
              <span className="text-muted me-2">
                {totalPartys} {totalPartys === 1 ? 'party' : 'partys'} found
              </span>
            </Col>
          </Row>
        </Card.Body>
        ...
        <Card.Body className="p-0">
          {loading ? (
            <div className="text-center py-5">
              ...
              <p className="mt-3">Loading partys...</p>
            </div>
          ) : partys.length === 0 ? (
            <div className="text-center py-5">
              <p className="mb-0">No partys found. Add your first party to get started.</p>
            </div>
          ) : (
