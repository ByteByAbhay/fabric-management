...
const VendorFormUpdated = ({ initialVendorData = null, vendorId = null, isEditMode = false }) => {
  const navigate = useNavigate();
  const { id: urlId } = useParams();
  const effectiveId = vendorId || urlId;
  const effectiveIsEditMode = isEditMode || !!effectiveId;
  
  // Debug information
  console.log('VendorFormUpdated component loaded');
  console.log('Props:', { initialVendorData, vendorId, isEditMode });
  ...
  useEffect(() => {
    const processVendorData = (vendorData) => {
      // Format date for input field
      vendorData.date_time = formatDateForInput(vendorData.date_time);
      
      // Ensure all color_hex values are set
      if (vendorData.fabricDetails && Array.isArray(vendorData.fabricDetails)) {
        vendorData.fabricDetails.forEach(fabric => {
      ...
      console.log('Processed vendor data for form:', vendorData);
    ...
    if (effectiveIsEditMode && effectiveId) {
      const fetchVendorData = async () => {
        try {
          ...
          console.log(`Fetching vendor data for ID: ${effectiveId}`);
  ...
  useEffect(() => {
    const fetchFabricData = async () => {
      try {
        // Get fabric types
        const typesResponse = await axios.get('/api/vendors/fabric-types');
        if (typesResponse.data.success) {
          setAvailableFabrics(typesResponse.data.data);
        }
        
        // Get fabric colors
        const colorsResponse = await axios.get('/api/vendors/fabric-colors');
  ...
  const handleSubmit = (e) => {
    ...
    console.log('Submitting vendor data:', updatedFormData);
...
export default VendorFormUpdated;
